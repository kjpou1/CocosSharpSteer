...
namespace CocosSharpSteer.Database
{
	...
	class LocalityQueryDatabase
	{
		 ...
		 * contents.
		 */
		public LocalityQueryDatabase(CCVector2 origin, CCVector2 size, int divx, int divy, int divz)
		{
			_origin = origin;
			_size = size;
			_divX = divx;
...
//			}
	    ...
	    private int BinCoordsToBinIndex(int ix, int iy)
		{
		...
		public void UpdateForNewLocation(ClientProxy obj, CCVector2 position)
		{
	    ...
	    private /*lqClientProxy*/int BinForLocation(CCVector2 position)
		{
...
//				position.X >= _origin.X + _size.X || position.Y >= _origin.Y + _size.Y || position.Z >= _origin.Z + _size.Z)
            if (position.X < _origin.X || position.Y < _origin.Y ||
                position.X >= _origin.X + _size.X || position.Y >= _origin.Y + _size.Y)
			{
			...
			int i = BinCoordsToBinIndex(ix, iy);
		...
		/* Apply an application-specific function to all objects in a certain
		   ...
		   bins of interest. */
		public void MapOverAllObjectsInLocality(CCVector2 center, float radius, LQCallBackFunction func, Object clientQueryState)
		{
			int partlyOut = 0;
			...
			int minBinZ = 0;//(int)((((center.Z - radius) - _origin.Z) / _size.Z) * _divZ);
			int maxBinX = (int)((((center.X + radius) - _origin.X) / _size.X) * _divX);
			int maxBinY = (int)((((center.Y + radius) - _origin.Y) / _size.Y) * _divY);
			int maxBinZ = 0;//(int)((((center.Z + radius) - _origin.Z) / _size.Z) * _divZ);
	    ...
	    private static void TraverseBinClientObjectList(ClientProxy co, float radiusSquared, LQCallBackFunction func, Object state, CCVector2 position)
		{
			while (co != null)
			{
				// compute distance (squared) from this client obj to given
				// locality sphere's centerpoint
				CCVector2 d = position - co.Position;
	    ...
	    private void MapOverAllObjectsInLocalityClipped(CCVector2 center, float radius,
		...
		{
		    int i;
            int slab = _divX;//_divY; //_divZ;
            int row = minBinX * slab;//_divX;//_divZ;
            int istart = minBinX * _divX;
            int jstart = minBinY;
			int kstart = minBinZ;
...
//					{
						...
						ClientProxy bin = _bins[iindex + j];
						ClientProxy co = bin;
...
//					}
        ...
        /// <param name="func"></param>
        /// <param name="clientQueryState"></param>
	    private void MapOverAllOutsideObjects(CCVector2 center, float radius, LQCallBackFunction func, Object clientQueryState)
		{
